...
echo "Operador AND: &&"
echo "Comando: (())"
echo "Operadores relacionais: == e != (No comando (()), para números)"
...
echo -ne "(( \"\$var\" != \"\$var\" && \"\$var\" == \"\$var\" )); echo \$?\t"; (( "$var" != "$var" && "$var" == "$var" )); echo $?	# 1
echo -ne "(( \"\$var\" != \"\$var\" && \"\$var\" != \"\$var\" )); echo \$?\t"; (( "$var" != "$var" && "$var" != "$var" )); echo $?	# 1
echo -e "\nAusência ou presença de aspas não tem impacto porque o valor da variável é numérico, não contém espaço vazio ou em branco."
